...
describe("Testing Auth Endpoints", () => {
  describe("Create a new account", () => {
    ...
    it("New user should have 'Classic' role", async function () {
      expect(createdUser.role).to.be.equals("Classic");
      expect(createdUser.role).to.be.a("string");
    });

    it("New user should have 'App' registerWith", async function () {
      expect(createdUser.registerWith).to.be.equals("App");
  ...
  describe("Login user", () => {
    ...
    it("Valid login should store cookie", async function () {
      ...
      const result = await requester.post("/api/v1/auth/login").send(mockUser);
  ...
  describe("Logout user", () => {
    it("Logout user successfully", function (done) {
      requester.get("/api/v1/auth/logout").end((error, response) => {
        if (error) {
          done(error);
        } else {
...
describe("Testing Product Endpoints", () => {
  ...
  describe("Get products", () => {
    it("Get all products", async function () {
      const { statusCode, ok, _body } = await requester.get("/api/v1/product");
...
describe("Testing Cart Endpoints", () => {
  describe("Create a new cart", () => {
    it("Create a new cart successfully", async function () {
      ...
      const result = await requester.post("/api/v1/product").send(mockProduct);
